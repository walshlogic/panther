...
class Vision{
    ...
    public
        ...
        try{
            ...
            $connection=new PDO("dblib:host={$server};database={$database}",
                    $user, $password);
    ...
    // ===========================
    private
            function get_parcel($parcel_number=""){
    ...
    private
        ...
        foreach($sketches as $sketch){
            ...
            $file_date=date(
                    "Y-m-d", filemtime("{$this->sketches_mount}/{$sketch}")
            );
            $duplicate=$this->checkForDuplicateSketch(
                    $parcel_id, $building_id, $file_date
            );
    ...
    // ============================
    private
            function getTaxGroupDescription($tax_group_description=""){
        $description='';
...
//        return $site_address;
    ...
    // =================================
    private
            function getReplacementValues($prx_pid=""){
        $SQL="select PSA_STREET_NUM_CHAR, PSA_STREET, PSA_STREET_PREFIX, PSA_CITY, PSA_STATE, PSA_ZIP
              from V7_VISION.COMMON.SITE_ADDRESS
             where PSA_SITE_ID = '{$site_id}'";
        $site_address=$this->vision_connection->query($SQL)->fetch(PDO::FETCH_ASSOC);
