...
function updateDatabase($pdo, $oldFilePath, $newFilePath, $newFileName, $bid)
{
    try {
        ...
        if ($stmt->execute()) {
            //echo "Database Updated Successfully.(ES-scf-139)\n";
        } else {
            //echo "Database Update Failed(ES-scf-141): " . json_encode($stmt->errorInfo()) . "\n";
            print_r($stmt->errorInfo());
        }
    }
    catch (PDOException $e) {
        //echo "Database Update Error(ES-scf-145): " . $e->getMessage() . "\n";
        print_r("Caught Execption: " . $e->getMessage());
    }
}
